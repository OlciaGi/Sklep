...
namespace SklepKsiegarniaInt.Controllers
{
    ...
    public class ManageController : Controller
    {
        ...
        {
            Ksiazka ksiazka;
            if (ksiazkaId.HasValue)
            {
                ViewBag.EditMode = true;
                ksiazka = db.Ksiazki.Find(ksiazkaId);
            }
            else
            {
            ...
            var result = new EditKsiazkiViewModel();
            result.Kategorie = db.Kategorie.ToList();
            result.Ksiazka = ksiazka;
            result.Potwierdzenie = potwierdzenie;
            return View(result);
        }
        [HttpPost]
        [Authorize(Roles = "Admin")]
        public ActionResult DodajKsiazke(EditKsiazkiViewModel model, HttpPostedFileBase file)
        {
            ...
            if (file != null && file.ContentLength > 0)
            {
                if (ModelState.IsValid)
                {
                    ...
                    model.Ksiazka.NazwaPlikuObrazka = filename;
                    model.Ksiazka.DataDodania = DateTime.Now;

                    db.Entry(model.Ksiazka).State = EntityState.Added;
                    db.SaveChanges();

                    return RedirectToAction("DodajKsiazke", new { potwierdzenie = true });
                }
                else
                {
